...
function SetupAndHeatmap() {
  return (
    <>
      {/* 3 steps to live */}
      <section className="py-8 px-4 sm:px-6 max-w-5xl mx-auto" style={{ borderTop: '1px solid rgba(59,130,246,0.08)' }}>
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3 flex-wrap">
          ...
          {[
            ...
            <div key={s.n} className="flex items-center gap-2 px-4 py-2 rounded-full text-sm text-slate-600" style={{ border: '1px solid rgba(59,130,246,0.14)', background: 'rgba(59,130,246,0.05)' }}>
              <span className="w-5 h-5 rounded-full text-blue-700 text-xs font-bold flex items-center justify-center flex-shrink-0" style={{ background: 'rgba(59,130,246,0.15)' }}>{s.n}</span>
              {s.label}
            </div>
          ))}
        </div>
      ...
      <section className="py-8 px-4 sm:px-6 max-w-5xl mx-auto">
        <div className="rounded-2xl p-6 bg-white shadow-sm" style={{ border: '1px solid rgba(59,130,246,0.10)' }}>
          <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
            ...
            <span className="text-[10px] font-bold px-3 py-1 rounded-full" style={{ border: '1px solid rgba(59,130,246,0.18)', color: '#3b82f6', background: 'rgba(59,130,246,0.07)' }}>Pro feature</span>
          </div>
          <div className="flex flex-col gap-3">
            {[
              { label: 'Screen & display repairs', pct: 68, color: 'rgba(59,130,246,0.70)'  },
              { label: 'Battery replacements',     pct: 51, color: 'rgba(5,150,105,0.70)'  },
              { label: 'Charging port issues',     pct: 33, color: 'rgba(29,78,216,0.70)' },
            ].map((row) => (
              <div key={row.label}>
