      ...
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < 90) {
            ...
            ctx.strokeStyle = `rgba(59,130,246,${0.07 * (1 - dist / 90)})`
            ctx.lineWidth = 0.5
            ctx.stroke()
          }
        }
  ...
  return (
    <>
      <style>{`
        ...
        .qt-card:hover { transform: translateY(-6px); box-shadow: 0 20px 40px rgba(59,130,246,0.15), 0 0 0 1px rgba(59,130,246,0.2); }
      ...
      <section style={{ position: 'relative', overflow: 'hidden', background: 'linear-gradient(180deg, #020617 0%, #030b18 100%)', padding: '80px 24px 60px' }}>
        <ParticleBg />
        <div style={{ position: 'absolute', top: -80, left: '25%', width: 500, height: 400, borderRadius: '50%', background: 'radial-gradient(circle, rgba(59,130,246,0.1) 0%, transparent 70%)', animation: 'qt-glow 7s ease-in-out infinite', pointerEvents: 'none' }} />
        ...
        <div style={{ position: 'relative', maxWidth: 720, margin: '0 auto', textAlign: 'center', opacity: visible ? 1 : 0, animation: visible ? 'qt-fade-up 0.6s ease-out' : 'none' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(59,130,246,0.1)', border: '1px solid rgba(59,130,246,0.25)', borderRadius: 99, padding: '6px 16px', marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            <a href="#news" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '13px 28px', borderRadius: 12, background: `linear-gradient(135deg, ${ACCENT}, ${ACCENT2})`, color: '#fff', fontWeight: 700, fontSize: 15, textDecoration: 'none', boxShadow: '0 4px 20px rgba(59,130,246,0.35)', transition: 'transform 150ms ease' }}
              onMouseEnter={e => (e.currentTarget as HTMLElement).style.transform = 'scale(1.04)'}
              onMouseLeave={e => (e.currentTarget as HTMLElement).style.transform = 'scale(1)'}
            >Read today&apos;s tech →</a>
            <a href="#how-it-works" style={{ display: 'inline-flex', alignItems: 'center', padding: '13px 24px', borderRadius: 12, border: '1px solid rgba(59,130,246,0.25)', color: ACCENT, fontWeight: 600, fontSize: 15, textDecoration: 'none', background: 'rgba(59,130,246,0.05)' }}>How it works</a>
          </div>
        </div>
      </section>
      ...
      <section style={{ background: '#020617', padding: '60px 24px' }}>
        <div style={{ maxWidth: 1080, margin: '0 auto' }}>
          ...
          <div className="qt-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 16 }}>
            {features.map((f, i) => (
              <div key={i} className="qt-card" style={{ padding: '24px', background: 'rgba(59,130,246,0.04)', border: '1px solid rgba(59,130,246,0.12)', borderRadius: 16, animation: `qt-fade-up 0.5s ease-out ${i * 0.1}s both` }}>
      ...
      <section id="how-it-works" style={{ background: 'linear-gradient(180deg, #020617 0%, #030b18 100%)', padding: '60px 24px 80px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          ...
          <div className="qt-steps" style={{ display: 'flex', gap: 0, position: 'relative' }}>
            <div style={{ position: 'absolute', top: 28, left: '12%', right: '12%', height: 1, background: 'rgba(59,130,246,0.12)', pointerEvents: 'none' }} />
            {steps.map((s, i) => (
              <div key={i} onClick={() => setActiveStep(i)} style={{ flex: 1, textAlign: 'center', padding: '0 12px', cursor: 'pointer' }}>
                <div style={{ width: 56, height: 56, borderRadius: '50%', background: activeStep === i ? `linear-gradient(135deg, ${ACCENT}, ${ACCENT2})` : 'rgba(59,130,246,0.08)', border: `2px solid ${activeStep === i ? ACCENT : 'rgba(59,130,246,0.2)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px', transition: 'all 300ms cubic-bezier(.23,1,.32,1)', boxShadow: activeStep === i ? '0 0 20px rgba(59,130,246,0.4)' : 'none' }}>
